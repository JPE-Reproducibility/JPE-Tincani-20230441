...
include("Functions_rescale_fast.jl")        
...
const size_shocks=20;                                        # number of shock draws
...
include(joinpath(path_codes, "Criterion_Function_rescale_fast_adjw.jl"))



## The following starting value was obtained from an optmization  with the bounds specified in lines 535-546, and with the following tolerances:
#     NLopt.xtol_rel!(opt, 1e-5)
#     NLopt.ftol_rel!(opt, 1e-5)
theta0 = [0.292333144,    # lambdaPR_0k, constant in utility from SUA enroll and drop out, type 1 high ability
0.760650378,             # lambdaPR_0k, constant in utility from SUA enroll and drop out, type 2 low ability
0.720013225,             # betaGPA_0k, constant in GPA production, type 1 high ability
1.152998564,             # betaGPA_0k, constant in GPA production, type 2 low ability
-0.122203916,            # betaPSU_0k, constant in PSU production, type 1 high ability
-1.250290015,            # betaPSU_0k, constant in PSU production, type 2 low ability
-0.015239229,            # theta_ppersist0k, constant in actual persistence, type 1 high ability
-0.108583235,            # theta_ppersist0k, constant in actual persistence, type 2 low ability
-0.006101898,  #         # xi_1k, linear term effort cost, type 1  high ability
-0.305288102,            # xi_1k, linear term effort cost, type 1  low ability
1.499492985,             # omega constant in single index for type 2 in type prob, higher value increases prob type 2. Wity more than 2 types, put omega_0 for type 3 right after this.
3.148711279,             # omega coeff on y_data_missing/survey_missing in single index for type 2 in type prob - larger values increase prob of type 2 for those with missing data. Wity more than 2 types, put omega_1 for type 3 right after this.
-0.343386623,            # omega coeff on female in single index for type 2 in type prob -- larger values increase prob of type 2 for females vs males. Wity more than 2 types, put omega_2 for type 3 right after this.
-3.882402457,            # omega coefficient on top15baseline_data in single index for type 2 in type prob -- larger values increase prob of type 2 for top15baseline vs bottom85 baseline, put omega_3 for type 3 right after this 
-4.158386942,            # zetabpace_0, constant in subj prob PACE admission
0.072775017, #           # zetabpace_1, coeff on perceived distance from cutoff in subj prob PACE admission
-0.006220098,            # xi_2, quadratic term effort cost -0.00026364,  
0.112691665,             # costSTreat, additional cost for taking PSU if in T
0.068405306,             # costS, cost of sitting PSU 
-0.408378881,            # gammab_0, constant in subj prob regular admission
9.862467068,             # gammab_1, coeff on PSUb in subj prob regular admission --> the larger this value, the lower the prob of regular admission in C group
0.895336693,             # delta, stigma from PACE enrollment    
0.229656724,             # lambdaG0, constant in graduation utility
0.131026169,             # sigma_error_eff, st dev of Normal m.e. on effort, measurement_err_eff
0.045190416,             # betaGPA_1, coeff on effort in GPA
0.851344706,             # betaGPA_2, coeff on GPA_avg_1_2 in GPA (6.0 for rank)
0.119878619,             # betaGPA_3, coeff on simce in GPA
0.345072401,             # sigma_eGPA, st dev of GPA shock
0.001227061,             # betaPSU_1, coeff on eff in PSU
0.051439317,             # betaPSU_2, coeff on GPA segundo in PSU
0.245557418,             # betaPSU_3, coeff on simce in PSU
0.045633873,             # sigma_ePSU, st dev of PSU shock
0.029722913,             # theta_ppersist[1], hours study in actual prob persist exog stoch process
0.586689335]             # theta_ppersist[2], simce in actual prob persist exog stoch process
      ...
      # Define bounds for all variables - local search for speed
      lower_bounds = theta0 .- 0.01 .* abs.(theta0)
      upper_bounds = theta0 .+ 0.01 .* abs.(theta0)


     # Set specific lower bounds for certain parameters - used to generate the starting value
    ...
    open(joinpath(path_processed, "estimation_output_rescale_fast_adjw_20shocks.txt"), "w") do io
    redirect_stdout(io)
    ...
    writedlm(joinpath(path_processed, "estimation_theta_rescale_fast_adjw_20shocks.csv"), min_x, ',')
end
