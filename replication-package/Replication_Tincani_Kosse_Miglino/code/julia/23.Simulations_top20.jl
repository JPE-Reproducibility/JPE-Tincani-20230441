# COUNTERFACTUAL: RE in both C and T schools
# (fixed point in each school treated school)



# We approximate the pace admission likelihood with a linear probability model, we keep all parameters fixed across 
# Fixed Point iterations except the coefficients on GPA_all_years
#NLopt, Plots,
import Pkg
Pkg.activate(@__DIR__)
using StatFiles, DataFrames, CSV, Distributions,  Random, Statistics, GLM #, GLM, QuantileRegressions #use packages
using LinearAlgebra
...
# Enrollment ut depends on true dropout prob

################################################################################
#                            Import and clean data                             #
################################################################################
### Import data
#Dataset generated by prob_top_5_25_for_RE.do in PACE/Do files
#includes all model initial conditions plus the variables that enter the 
#top20 probability approximation, i.e. the variables kept from a LASSO reg
const data = DataFrame(load(joinpath(path_processed, "initial_conditions_CC_rescale_RE_with_bias_top_20.dta")))

### Import parameters estimated outside the model
const param_outside_matrix = DataFrame(load(joinpath(path_processed, "parameters_estimated_outside_rescale.dta")))
const param_outside_vector=Array{Float64}(param_outside_matrix[:, "param_values"]);
...
const x_val0_data = data[:, "x_val0"] 
const coeff_GPA_all_years_data = data[:,"coeff_GPA_all_years"]; #initial guess for FP algorithm
...
# Define sizes of heterogeneous types, shocks and effort points
const size_K=2;   #number of unobserved heter. grid points
const size_shocks=20;   #number of shock realizations -- always keep at one for RE simulations
const size_Eff=11;   #number of grid points for effort
# Create the grid for effort
...
#                               Fixed point loop                               #
# ==============================================================================
# The fixed point loop finds the rational expectation probability (approximated equilibrium)
# It solves for the coefficient on GPA_all_years in the prob of being in the 
# top 20% that is closest to the grid point to which we are best responding


const size_grid_iter=100;



# List of treated students
treated_inds = findall(pace_data .== 1)

# List of treated school IDs
treated_school_ids = sort(unique(rbd[treated_inds]))
school_idx = Dict(s => i for (i, s) in enumerate(treated_school_ids))
num_schools = length(treated_school_ids)


# Storage: rows = schools, cols = iterations
coeff_matrix = fill(NaN, num_schools, size_grid_iter);

# We set a range for the guesses from 0 to approximately data estimate+ 2* data estimate, where 
# ata estimate is the estimated coeff of GPA_all_years (LPM):  .6748118 
const iter_grid=collect(linspace(0.0,1.0,size_grid_iter))       
iter_sim=zeros(size_grid_iter) ; #, num_treated_schools);   # initialize matrix of simulated GPA_all_years coefficients: one for each grid point


intop20_mc=zeros(size_N,size_shocks,size_grid_iter);
GPA_sim_mc=zeros(size_N,size_shocks,size_grid_iter);

# ============================================
# Start grid iterations 
# ============================================

for c=1:1:size_grid_iter


# ------------------------------------------------------------------------
#              Model simulation at current grid point         
# ------------------------------------------------------------------------
...
for i=1:1:size_N   # for each student i
   ...
   for m=1:1:size_shocks 
    ...
    for e=1:1:size_Eff   # for each level of effort
        ...
        expGPAb=ExpGPA(k,GPA_avg_1_2,simce,eff,betaGPA_0,betaGPA_1,betaGPA_2,betaGPA_3)   # Rational Expectations 
        expGPAb_912= 0.5*GPA_avg_1_2+0.5*expGPAb
        expPSUb= ExpPSU(k,GPA_avg_1_2,simce,eff,betaPSU_0,betaPSU_1,betaPSU_2, betaPSU_3)  # Rational Expectations 
        ...
        Pr_AdmR_b=cdf.(dist_stNormal,gammat_0+gammat_1*expPSUb+gammat_2*expPSUb*expPSUb+gammat_3*expPSUb*expPSUb*expPSUb) # rational expectations 
       
        x_val0=x_val0_data[i] 
        

        Pr_PACEslot_b=treated * max.(min.( x_val0+ iter_grid[c] * expGPAb_912, 1.0), 0.0) ## rational expectations 
        Pr_adm_eff[e]=(1.0-treated)*Pr_AdmR_b+treated*(Pr_AdmR_b+Pr_PACEslot_b-Pr_PACEslot_b*Pr_AdmR_b)
        
        p_grad = p_persist_actual(k,simce,eff,theta_ppersist0,theta_ppersist) ## enters value functions 
    ...
    GPA_sim_mc[i,m,c]= 0.5*GPA_avg_1_2+0.5*max.(min.(ExpGPA(k,GPA_avg_1_2,simce,Eff_sim[i,m],betaGPA_0,betaGPA_1,betaGPA_2,betaGPA_3)+eGPA[i,m],7.0),1.0)[1];    # Simulated GPA all years at grid point cc, including shocks
...
for i=1:1:size_N   # for each student i
   ...
   Pr_PACEslot_b_obs_sim[i,m]=max.(min.( x_val0 + iter_grid[c] * expGPAb_912, 1.0), 0.0)
   Pr_PACEslot_b_sim[i,m]=treated * Pr_PACEslot_b_obs_sim[i,m] ## rational expectations
   ...
   Pr_AdmR_b=cdf.(dist_stNormal,gammat_0+gammat_1*expPSUb+gammat_2*expPSUb*expPSUb+gammat_3*expPSUb*expPSUb*expPSUb) # rational expectations 
   Pr_Admitted_R_b_sim[i,m]=Pr_AdmR_b
...
cutoff_top20_sim=zeros(size_N, size_shocks);   # initialize matrix of simulated top 20 cutoff
...
for m=1:1:size_shocks 
   element_pos[1]=1
   for i=1:1:(size_N-1)
      cutoff_top20_sim[i,m]=quantile!(GPA_all_sim[element_pos[1]:(element_pos[1]-1+Int(n_obs_school[i])),m],0.80)
      if rbd[i]==rbd[i+1]
         else
      element_pos[1]=element_pos[1]+Int(n_obs_school[i])
      end
   end # for i 
   cutoff_top20_sim[size_N,m]=cutoff_top20_sim[size_N-1,m]
   end # for m 



## Time period 3: PACE admission
PACEslot_sim=zeros(size_N, size_shocks);   # initialize matrix of simulated PACE slot
intop20_sim=zeros(size_N,size_shocks);   # initialize matrix of being in top 20
...
for i=1:1:size_N   # for each student i
    ...
    if GPA_all_sim[i,m]>cutoff_top20_sim[i,m]
       PACEslot_sim[i,m]=treated*sit_PSU;
       intop20_sim[i,m]=treated;
    else
       PACEslot_sim[i,m]=0.0;
       intop20_sim[i,m]=0.0;
    end
    AdmP_sim[i,m]=PACEslot_sim[i,m]
    # adjusted admission to use as regression outcome -- keeping other LPM parameters fixed through the iterations
    #                                                    estimating only coeff on GPA_all_years
    x_val0=x_val0_data[i] 
    
    intop20_mc[i,m,c]=intop20_sim[i,m]-(x_val0)
end   # for m
end # for i



 # Collect (i, m) level data for this iteration
 df_rows = []

 for i in treated_inds
     school = rbd[i]
     for m in 1:size_shocks
         gpa = GPA_sim_mc[i, m, c]
         intop20 = intop20_mc[i, m, c]
         push!(df_rows, (
             school_id = school,
             GPA_dataS = gpa,
             intop20_cS = intop20
 ...
 # Within-school regressions
 for school in treated_school_ids
     df_school = filter(:school_id => ==(school), df)
     if nrow(df_school) > 1
         reg = lm(@formula(intop20_cS ~ 0 + GPA_dataS), df_school)
         coeff_matrix[school_idx[school], c] = coef(reg)[1]
     end
 end


end   # for c (end of fixed point loop)



################################################################################
#                    Save predicted iter  and initial iter                   #
################################################################################
using DataFrames
using CSV

# Transpose coeff_matrix so that each column corresponds to a school, and each row to an iteration
# This is now (size_grid_iter, num_schools)
coeff_matrix_T = transpose(coeff_matrix)

# Build output matrix
Fixed_point_output = zeros(size_grid_iter, 1 + num_schools)

# First column = iter_grid
Fixed_point_output[:, 1] = iter_grid[:]

# Remaining columns = school coefficients for each iteration
   Fixed_point_output[:, 2:end] = coeff_matrix_T

   # Create DataFrame and export
df = DataFrame(Fixed_point_output, :auto)

CSV.write(joinpath(path_processed, "fixed_point_by_school_RE_top20_value_effort.csv"), df)



coeff_GPAallyears_RE=zeros(size_N);   # initialize vector of rational expectation cutoff

# Find fixed point 
for i in 1:size_N
   if pace_data[i] == 1
       school = rbd[i]
       school_row = school_idx[school]  # row in coeff_matrix
       diffs = abs.(coeff_matrix[school_row, :] .- iter_grid)
       c_star = argmin(diffs)
       coeff_GPAallyears_RE[i] = iter_grid[c_star]
   end
end
...
for i=1:1:size_N   # for each student i
   ...
   for m=1:1:size_shocks 
    ...
    for e=1:1:size_Eff   # for each level of effort
      ...
      Pr_PACEslot_b=treated * max.(min.( x_val0+ coeff_GPAallyears_RE[i] * expGPAb_912, 1.0), 0.0) ## rational expectations 
...
for i=1:1:size_N   # for each student i
   ...
   Pr_PACEslot_b_obs_sim[i,m]=max.(min.( x_val0 +  coeff_GPAallyears_RE[i] * expGPAb_912, 1.0), 0.0)
   Pr_PACEslot_b_sim[i,m]=treated * Pr_PACEslot_b_obs_sim[i,m] ## rational expectations
   Pr_PACEslot_b=Pr_PACEslot_b_sim[i,m];
...
PACEslot_sim=zeros(size_N, size_shocks);   # initialize matrix of simulated PACE slot
AdmP_sim=zeros(size_N, size_shocks);   # initialize matrix of admission
top20_sim=zeros(size_N, size_shocks);
   ...
   eff = Eff_sim[i,m]
    ...
    elseif (admr==1 && admp==0)   # if regular admission only
        if V_R>=V_0
           ...
           Ut_5_ante_RE_sim[i,m]=V_R  # they alrady have RE
        else
           E0_sim[i,m]=1.0
           ER_sim[i,m]=0.0
           EP_sim[i,m]=0.0
     ...
     elseif (admr==0 && admp==1)   # if PACE admission only
         if V_P>=V_0
            ...
            Ut_5_ante_RE_sim[i,m]=V_P # they already have RE
         else
            E0_sim[i,m]=1.0
            ER_sim[i,m]=0.0
            EP_sim[i,m]=0.0
...
for m=1:1:size_shocks 
   ...
   # identifiers
    ...
    Simulation_Output[i,m,10]=AdmP_sim[i,m]       # X sim_admitted_SUA_pace (= in top 20% and take PSU)
    ...
    Simulation_Output[i,m,15]=Pr_PACEslot_b_sim[i,m]            # X simulated subjective probability of being in top 20 percent of school, latent
    ...
    #simulated being in the top 20
    Simulation_Output[i,m,35]=top20_sim[i,m]
...
## save dataset with cutoffs
Simulation_cutoff=Array{Union{Missing, Float64},2}(undef,size_N,size_shocks)

# Build a long-format DataFrame: one row per (i, m)
cutoff_rows = []
...
for m in 1:size_shocks
    for i in 1:size_N
        push!(cutoff_rows, (
            ...
            shock = m,
            treated = pace_data[i],
            cutoff = cutoff_top20_sim[i, m]
        ))
    end
end

df_cutoff = DataFrame(cutoff_rows)

CSV.write(joinpath(path_processed, "countRE_CC_cutoffs_top20.csv"), df_cutoff)


# Recover cutoff biases
colnames = ["biasREcutoff$(i)" for i in 1:size_shocks]
const biasREcutoff = Matrix{Float64}(coalesce.(Matrix(data[:, colnames]), 0.0))
# Generate cutoff beliefs, summing RE equilibrium cutoff to bias with respect to RE equilibrium cutoff
cutoff_b=cutoff_top20_sim+biasREcutoff
...
for i=1:1:size_N   # for each student i
   ...
   for m=1:1:size_shocks 
    k=k_vec[i,m];    # assign type
    expCutoff_top20_b=cutoff_b[i,m];   # perceived expected cut-off
    ...
    for e=1:1:size_Eff   # for each level of effort
        ...
        Pr_PACEslot_b=treated*PrPACEslotb(dist_stNormal,zetabpace_0,zetabpace_1,expGPAb_912,expCutoff_top20_b) # prob. of PACE admission
        Pr_adm_eff[e]=(1.0-treated)*Pr_AdmR_b+treated*(Pr_AdmR_b+Pr_PACEslot_b-Pr_PACEslot_b*Pr_AdmR_b)
...
for i=1:1:size_N   # for each student i
   ...
   expGPAb_sim[i,m]=expGPAb
   expCutoff_top20_b=cutoff_b[i,m];   # perceived expected cut-off
   Pr_PACEslot_b_sim[i,m]=treated*PrPACEslotb(dist_stNormal,zetabpace_0,zetabpace_1,expGPAb_912,expCutoff_top20_b); # subjective prob. of PACE admission
   Pr_PACEslot_b_obs_sim[i,m]=PrPACEslotb(dist_stNormal,zetabpace_0,zetabpace_1,expGPAb_912,expCutoff_top20_b)
   Pr_PACEslot_b=Pr_PACEslot_b_sim[i,m];
...
# Print the dataset with mrun and simulated data
## print dataset
Simulation_Output=Array{Union{Missing, Float64},3}(undef,size_N,size_shocks,83)
for m=1:1:size_shocks 
   for i=1:1:size_N
   # identifiers
    Simulation_Output[i,m,1]=data[i,2]    # X mrun
    ...
    for e=1:1:size_Eff
      ...
      Simulation_Output[i,m,81]=Ut_1_sim[i,m]
      Simulation_Output[i,m,82]=Ut_2_sim[i,m]
      Simulation_Output[i,m,83]=Ut_5_post_sim[i,m]
...
CSV.write(joinpath(path_processed, "countRE_top20_CC_2types.csv"), df)
