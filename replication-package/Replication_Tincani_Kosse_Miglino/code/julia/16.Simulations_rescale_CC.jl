...
# Import data
const data = DataFrame(load(joinpath(path_processed, "initial_conditions_CC_rescale.dta")))
...
for i=1:1:size_N
   ...
   #alumno_prioritario=alumno_prioritario_data[i]
   k=k_vec[i,m]
   ...
   elseif (enrolled_regular==1 && persist==0)
      Ut_5_post_sim[i,m]=lambdaPR_0[k]+nuR[i,m]
...
if size_K == 1
    CSV.write(joinpath(path_processed, "baseline_CC_1type_rescale.csv"), df)
elseif size_K == 2
    CSV.write(joinpath(path_processed, "baseline_CC_2types_rescale.csv"), df)
elseif size_K == 3
    CSV.write(joinpath(path_processed, "baseline_CC_3types_rescale.csv"), df)
end
