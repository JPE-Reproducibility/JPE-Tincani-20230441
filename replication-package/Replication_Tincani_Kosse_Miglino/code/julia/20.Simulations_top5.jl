...
#top5 probability approximation, i.e. the variables kept from a LASSO reg
const data = DataFrame(load(joinpath(path_processed, "initial_conditions_CC_rescale_RE_with_bias_top_5.dta")))
...
# top 5% that is closest to the grid point to which we are best responding
...
# ata estimate is the estimated coeff of GPA_all_years (LPM)
...
intop5_mc=zeros(size_N,size_shocks,size_grid_iter);
...
cutoff_top5_sim=zeros(size_N, size_shocks);   # initialize matrix of simulated top 5 cutoff
...
for m=1:1:size_shocks 
   element_pos[1]=1
   for i=1:1:(size_N-1)
      cutoff_top5_sim[i,m]=quantile!(GPA_all_sim[element_pos[1]:(element_pos[1]-1+Int(n_obs_school[i])),m],0.95)
      if rbd[i]==rbd[i+1]
         else
      element_pos[1]=element_pos[1]+Int(n_obs_school[i])
      end
   end # for i 
   cutoff_top5_sim[size_N,m]=cutoff_top5_sim[size_N-1,m]
   end # for m 
...
intop5_sim=zeros(size_N,size_shocks);   # initialize matrix of being in top 5
...
for i=1:1:size_N   # for each student i
    ...
    if GPA_all_sim[i,m]>cutoff_top5_sim[i,m]
       PACEslot_sim[i,m]=treated*sit_PSU;
       intop5_sim[i,m]=treated;
    else
    ...
    intop5_mc[i,m,c]=intop5_sim[i,m]-(x_val0)
end   # for m
end # for i
 ...
 for school in treated_school_ids
     ...
     if nrow(df_school) > 1
         reg = lm(@formula(intop5_cS ~ 0 + GPA_dataS), df_school)
...
CSV.write(joinpath(path_processed, "fixed_point_by_school_RE_top5_value_effort.csv"), df)
...
PACEslot_sim=zeros(size_N, size_shocks);   # initialize matrix of simulated PACE slot
AdmP_sim=zeros(size_N, size_shocks);   # initialize matrix of admission
top5_sim=zeros(size_N, size_shocks);
...
for m=1:1:size_shocks 
   ...
   # identifiers
    ...
    Simulation_Output[i,m,10]=AdmP_sim[i,m]       # X sim_admitted_SUA_pace (= in top 5% and take PSU)
    ...
    Simulation_Output[i,m,15]=Pr_PACEslot_b_sim[i,m]            # X simulated subjective probability of being in top 5 percent of school, latent
    ...
    #simulated being in the top 5
    Simulation_Output[i,m,35]=top5_sim[i,m]
...
CSV.write(joinpath(path_processed, "countRE_CC_cutoffs_top5.csv"), df_cutoff)
...
cutoff_b=cutoff_top5_sim+biasREcutoff
...
for i=1:1:size_N   # for each student i
   ...
   for m=1:1:size_shocks 
    k=k_vec[i,m];    # assign type
    expCutoff_top5_b=cutoff_b[i,m];   # perceived expected cut-off
    ...
    for e=1:1:size_Eff   # for each level of effort
        ...
        Pr_PACEslot_b=treated*PrPACEslotb(dist_stNormal,zetabpace_0,zetabpace_1,expGPAb_912,expCutoff_top5_b) # prob. of PACE admission
        Pr_adm_eff[e]=(1.0-treated)*Pr_AdmR_b+treated*(Pr_AdmR_b+Pr_PACEslot_b-Pr_PACEslot_b*Pr_AdmR_b)
...
for i=1:1:size_N   # for each student i
   ...
   expGPAb_sim[i,m]=expGPAb
   expCutoff_top5_b=cutoff_b[i,m];   # perceived expected cut-off
   Pr_PACEslot_b_sim[i,m]=treated*PrPACEslotb(dist_stNormal,zetabpace_0,zetabpace_1,expGPAb_912,expCutoff_top5_b); # subjective prob. of PACE admission
   Pr_PACEslot_b_obs_sim[i,m]=PrPACEslotb(dist_stNormal,zetabpace_0,zetabpace_1,expGPAb_912,expCutoff_top5_b)
   Pr_PACEslot_b=Pr_PACEslot_b_sim[i,m];
...
CSV.write(joinpath(path_processed, "countRE_top5_CC_2types.csv"), df)
