...
################################################################################
### Import data
#Dataset generated by prob_top_15_fit.do in PACE/Do files
#includes all model initial conditions plus the variables that enter the 
#top15 probability approximation, i.e. the variables kept from a LASSO reg
const data = DataFrame(load(joinpath(path_processed, "initial_conditions_CC_rescale_RE_with_bias_top_15.dta")))
...
if length(theta) != expected_len
   ...
   println("Parameters values are $theta.")   # print the parameters values at each iteration
...
# top 15% that is closest to the grid point to which we are best responding
...
intop15_mc=zeros(size_N,size_shocks,size_grid_iter);
...
intop15_sim=zeros(size_N,size_shocks);   # initialize matrix of being in top 15
...
for i=1:1:size_N   # for each student i
    ...
    intop15_mc[i,m,c]=intop15_sim[i,m]-(x_val0)
end   # for m
end # for i
 ...
 for school in treated_school_ids
     ...
     if nrow(df_school) > 1
         reg = lm(@formula(intop15_cS ~ 0 + GPA_dataS), df_school)
...
CSV.write(joinpath(path_processed, "fixed_point_by_school_RE_top15_value_effort.csv"), df)
...
CSV.write(joinpath(path_processed, "countRE_CC_cutoffs_top15.csv"), df_cutoff)
...
cutoff_b=cutoff_top15_sim+biasREcutoff
...
for i=1:1:size_N   # for each student i
   ...
   for m=1:1:size_shocks 
    k=k_vec[i,m];    # assign type
    expCutoff_top15_b=cutoff_b[i,m];   # perceived expected cut-off
...
CSV.write(joinpath(path_processed, "countRE_top15_CC_2types.csv"), df)
