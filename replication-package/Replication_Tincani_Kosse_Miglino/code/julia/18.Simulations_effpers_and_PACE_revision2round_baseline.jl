...
# All students are simulated to have RE about the 
# likelihood of persisting, but we leave biased 
...
include(joinpath(path_codes, "Functions_RE.jl"))      
...
# Import data
const data = DataFrame(load(joinpath(path_processed, "initial_conditions_CC_rescale_RE_top_15.dta")))
...
# Baseline effort generated in 15.Simulations_rescale.jl:
const eff_sim_baseline = Matrix(CSV.read(joinpath(path_processed, "Eff_sim_baseline.csv"),DataFrame;header=false))
...
const size_K=2;                          # number of unobserved heter. grid points
const size_shocks=20;                    # number of shock realizations
...
# Read theta from the dedicated structured output written by the estimation script
function read_theta_from_structured_file(path)
    theta = vec(readdlm(path, ',', Float64))
    return theta
end

# Choose the structured estimation output file to read
estimation_output_path = joinpath(path_processed, "estimation_theta_rescale_fast_adjw_20shocks.csv")

theta = read_theta_from_structured_file(estimation_output_path)

# Sanity check: expected length for size_K=2 is 9*size_K+16 = 34
...
# One shock per person. Shocks are the same between two groups CC here
...
# equalize shocks in two groups CC
const size_halfN = div(size_N, 2)
params = [psi,ePSUreg, nuR, nuP, latent_type_sim, latent_dropout_sim, eGPA, ePSU, shock_selP, shock_selR, measurement_err_eff]
for param in params
    param[size_halfN+1:size_N, :] .= param[1:size_halfN, :]
end
...
for i=1:1:size_N   # for each student i
   ...
   for m=1:1:size_shocks 
    ...
    for e=1:1:size_Eff   # for each level of effort
        ...
        z = theta_ppersist0[k] + theta_ppersist[1] * eff + theta_ppersist[2] * simce
        p_grad = max.(min.(Pgradb_data[i]-eff_sim_baseline[i,m]* pdf(dist_stNormal, z) * theta_ppersist[1] + eff * pdf(dist_stNormal, z) * theta_ppersist[1],1.0) ,0.0); # derivative of persistence w.r.t effort is pdf(dist_stNormal, z) * theta_ppersist[1]
...
Pr_persist_sim=zeros(size_N,size_shocks);
...
for i=1:1:size_N   # for each student i
   ...
   z = theta_ppersist0[k] + theta_ppersist[1] * eff + theta_ppersist[2] * simce
   p_grad =  max.(min.(Pgradb_data[i] -eff_sim_baseline[i,m]* pdf(dist_stNormal, z) * theta_ppersist[1]+ eff * pdf(dist_stNormal, z) * theta_ppersist[1],1.0) ,0.0);
   Pr_persist_sim[i,m]=p_grad;
   expGPAb=ExpGPAb(GPA_avg_1_2,simce,eff,betaGPAb_0,betaGPAb_1,betaGPAb_2,betaGPAb_3);   # (expected) GPA 
   expGPAb_912=0.5*GPA_avg_1_2+0.5*expGPAb
...
Ut_5_ante_b_sim=zeros(size_N, size_shocks);
Ut_5_ante_RE_sim=zeros(size_N, size_shocks);
Ut_5_post_sim=zeros(size_N, size_shocks);
...
Pr_persist_RE_sim=zeros(size_N, size_shocks); # Objective prob. of persistence
...
for i=1:1:size_N  
   ...
   for m=1:1:size_shocks 
    ...
    # Calculate expected value using true persistence prob 
    p_grad_RE=p_persist_actual(k,simce,eff,theta_ppersist0,theta_ppersist) ## enters value functions 
    Pr_persist_RE_sim[i,m]=p_grad_RE
    V_R_RE=expVR(k,lambdaPR_0,lambdaG0,lambdaG1,selectivity_R_sim[i,m],p_grad_RE)+nuR[i,m];
    V_P_RE=expVP(k,lambdaPR_0,lambdaG0,lambdaG1,selectivity_P_sim[i,m],delta,p_grad_RE)+nuP[i,m];   # value of PACE enrolment
    ...
    elseif (admr==1 && admp==0)   # if regular admission only
        if V_R>=V_0
           ...
           ER_sim[i,m]=1.0
           EP_sim[i,m]=0.0
           Ut_5_ante_b_sim[i,m]=V_R
           Ut_5_ante_RE_sim[i,m]=V_R_RE
        else
           E0_sim[i,m]=1.0
           ER_sim[i,m]=0.0
           EP_sim[i,m]=0.0
     ...
     elseif (admr==0 && admp==1)   # if PACE admission only
         if V_P>=V_0
            ...
            ER_sim[i,m]=0.0
            EP_sim[i,m]=1.0
            Ut_5_ante_b_sim[i,m]=V_P
            Ut_5_ante_RE_sim[i,m]=V_P_RE
         else
            E0_sim[i,m]=1.0
            ER_sim[i,m]=0.0
            EP_sim[i,m]=0.0
     ...
     Utility_sim_expected[i,m]=Ut_1_sim[i,m]+Ut_2_sim[i,m]+Ut_5_ante_b_sim[i,m]
   end # for m
end   # for i
...
for i=1:1:size_N
   ...
   if (enrolled_pace==1.0 && persist==1)
      Ut_5_post_sim[i,m]=lambdaPR_0[k]-delta+(lambdaG0+lambdaG1.*selectivity_P_sim[i,m])+nuP[i,m]
   ...
   elseif (enrolled_pace==1 && persist==0)
      Ut_5_post_sim[i,m]=lambdaPR_0[k]-delta+nuP[i,m]
      Utility_sim_actual[i,m]=Ut_1_sim[i,m]+Ut_2_sim[i,m]+lambdaPR_0[k]-delta+nuP[i,m]
   elseif (enrolled_regular==1 && persist==1)
      Ut_5_post_sim[i,m]=lambdaPR_0[k]+(lambdaG0+lambdaG1.*selectivity_R_sim[i,m])+nuR[i,m]
   ...
   elseif (enrolled_regular==1 && persist==0)
      Ut_5_post_sim[i,m]=Ut_2_sim[i,m]+lambdaPR_0[k]+nuR[i,m]
...
## print dataset
Simulation_Output=Array{Union{Missing, Float64},3}(undef,size_N,size_shocks,85)
for m=1:1:size_shocks 
   for i=1:1:size_N
   # identifiers
    ...
    Simulation_Output[i,m,18]= Pr_persist_sim[i,m]          # X 
    ...
    # Period by period utilities 
    Simulation_Output[i,m,81]=Ut_1_sim[i,m];
    Simulation_Output[i,m,82]=Ut_2_sim[i,m];
    Simulation_Output[i,m,83]=Ut_5_ante_b_sim[i,m];
    Simulation_Output[i,m,84]=Ut_5_ante_RE_sim[i,m];
    Simulation_Output[i,m,85]=Ut_5_post_sim[i,m];
...
df = DataFrame(reshape(Simulation_Output, :, size(Simulation_Output, 3)), :auto)
   ...
   CSV.write(joinpath(path_processed, "counteffpersandPACE_CC_2types.csv"), df)



# Compare simulated perceived probability of graduating with actual probability of graduating
mean(Pr_persist_sim[:,:])-mean(Pr_persist_RE_sim[:,:])
