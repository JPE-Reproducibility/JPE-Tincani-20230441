...
# Import data
   const data=DataFrame(load(joinpath(path_processed, "initial_conditions_TC_bootstrap17.dta"))); 
...
const matrix_empirical_moments=DataFrame(load(joinpath(path_processed, "empirical_coefficients_rescale_bootstrap17.dta")));
...
Gradientbased=false
    ...
    using NLopt
     ...
     # write logs 
      open(joinpath(path_processed, "estimation_output_s_bootstrap17.txt"), "a") do io
      ...
      writedlm(joinpath(path_processed, "estimation_theta_s_bootstrap17.csv"), min_x, ',')
end   # do io 
