...
function linspace(a,b,c)
  ...
  # Create mask where eff >= PSUb_kink
  mask = eff .>= PSUb_kink
  # Compute values using vectorized conditional selection
  beta_eff = ifelse.(mask, betaPSUb_2, betaPSUb_1)
  # Compute the final output vectorized
  xpPSU = betaPSUb_0 .+ beta_eff .* eff .+ betaPSUb_3 .* GPA_avg_1_2 .+ betaPSUb_4 .* simce
  return xpPSU
  end
...
selP_output = lambdaP_0 .+ lambdaP_1 .* GPA_cuarto .+ lambdaP_2 .* GPA_cuarto .^ 2 .+
lambdaP_3 .* simce .+ lambdaP_4 .* simce .^ 2 .+ lambdaP_5 .* academic .* simce .+
lambdaP_6 .* academic .+ lambdaP_7 .* r4 .+ lambdaP_8 .* r5 .+ lambdaP_9 .* r7 .+
lambdaP_10 .* r8 .+ lambdaP_11 .* r10 .+ lambdaP_12 .* r13 .+ lambdaP_13 .* r14 .+
lambdaP_14 .* r15
return selP_output
end
  ...
  # If it uses the actual PSU as an input, it returns the actual expected selectivity.
   ...
   function selR(lambdaR_0, lambdaR_1, lambdaR_2, lambdaR_3, lambdaR_4, 
    lambdaR_5, lambdaR_6, lambdaR_7, lambdaR_8, lambdaR_9, 
    lambdaR_10, lambdaR_11, lambdaR_12, lambdaR_13, lambdaR_14, 
    PSU, simce, academic, r4, r5, 
    r7, r8, r10, r13, r14, r15)
# Vectorized computation
selR_output= lambdaR_0 .+ lambdaR_1 .* PSU .+ lambdaR_2 .* PSU .^ 2 .+
lambdaR_3 .* simce .+ lambdaR_4 .* simce .^ 2 .+ lambdaR_5 .* academic .* simce .+
lambdaR_6 .* academic .+ lambdaR_7 .* r4 .+ lambdaR_8 .* r5 .+ lambdaR_9 .* r7 .+
lambdaR_10 .* r8 .+ lambdaR_11 .* r10 .+ lambdaR_12 .* r13 .+ lambdaR_13 .* r14 .+
lambdaR_14 .* r15
return selR_output
  ...
  PrAdmRbres=cdf.(dist_stNormal,gammab_0.+gammab_1.*expPSUb);
  return PrAdmRbres
  end
  ...
  PrPACEslotbres=cdf.(dist_stNormal,zetabpace_0.+zetabpace_1.*(expGPAb.-expCutoff_top15_b)) # prob. of PACE admission
  ...
  xi_1val = getindex.(Ref(xi_1), k)
  effsq=eff.^2;
  U1res=xi_1val.*eff.+xi_2.*effsq; # flow utility in period 1
  return U1res
  end
  ...
  lambdaPR_0val = getindex.(Ref(lambdaPR_0), k)
  expVRres=lambdaPR_0val.+p_grad.*(lambdaG0.+lambdaG1.*expselR); # expected value of regular enrolment
  return expVRres
  end
  ...
  lambdaPR_0val = getindex.(Ref(lambdaPR_0), k)
  expVPres=lambdaPR_0val.-delta.+p_grad.*(lambdaG0.+lambdaG1.*expselP);   # expected value of PACE enrolment
  return expVPres
  end
  ...
  function Emax_S_fun(k,treated,expselP,expselR,costS,costSTreat,lambdaPR_0,lambdaG0,lambdaG1,delta,Pr_AdmR_b,Pr_PACEslot_b,p_grad)
    expV_0=0.0.+Emax_E_fun(k,expselP,expselR,0.0,0.0,lambdaPR_0,lambdaG0,lambdaG1,delta,p_grad);   # value of not sitting the PSU
    expV_S=U2(costS,costSTreat,treated).+(1 .-Pr_AdmR_b).*(1 .-Pr_PACEslot_b).*Emax_E_fun(k,expselP,expselR,0.0,0.0,lambdaPR_0,lambdaG0,lambdaG1,delta,p_grad).+
           (1 .-Pr_AdmR_b).*Pr_PACEslot_b.*(Emax_E_fun(k,expselP,expselR,0.0,1.0,lambdaPR_0,lambdaG0,lambdaG1,delta,p_grad)).+
            Pr_AdmR_b.*(1 .-Pr_PACEslot_b).*Emax_E_fun(k,expselP,expselR,1.0,0.0,lambdaPR_0,lambdaG0,lambdaG1,delta,p_grad).+
            Pr_AdmR_b.*Pr_PACEslot_b.*(Emax_E_fun(k,expselP,expselR,1.0,1.0,lambdaPR_0,lambdaG0,lambdaG1,delta,p_grad));   # expected value of sitting the PSU
  ...
  function Emax_E_fun(k,expselP,expselR,AdmR,AdmP,lambdaPR_0,lambdaG0,lambdaG1,delta,p_grad)  # define type    
    ...
    expV_0=0.0.*expV_P;   # value of no enrolment (deterministic part)
  ...
  function p_persist_actual(k,simce,eff,theta_ppersist0,theta_ppersist)
    theta_ppersist0val = getindex.(Ref(theta_ppersist0), k)
  p_persist_actual_out=cdf(dist_stNormal,theta_ppersist0val.+theta_ppersist[1].*eff.+theta_ppersist[2].*simce)
