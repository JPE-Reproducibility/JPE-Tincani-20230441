...
for i=1:1:size_N   # for each student i
   ...
   Pr_persist_sim[i,m]=p_persist_actual(k,simce,eff,theta_ppersist0,theta_ppersist);
   expGPAb=ExpGPAb(GPA_avg_1_2,simce,eff,betaGPAb_0,betaGPAb_1,betaGPAb_2,betaGPAb_3);   # (expected) GPA 
   expGPAb_912=0.5*GPA_avg_1_2+0.5*expGPAb
...
Ut_enroll_dropout_R=zeros(size_N, size_shocks);  # experienced utility from enrolling and dropping out R
Ut_enroll_dropout_P=zeros(size_N, size_shocks);  # experience utility from enrolling and dropping out P
Ut_enroll_grad_R=zeros(size_N, size_shocks);  #experienced utility from enrolling and graduating R
Ut_enroll_grad_P=zeros(size_N, size_shocks);  #experienced utility from enrolling and graduating P
...
for i=1:1:size_N  
   ...
   for m=1:1:size_shocks 
    ...
    Ut_enroll_dropout_R[i,m]=lambdaPR_0[k]+nuR[i,m]
    Ut_enroll_dropout_P[i,m]=lambdaPR_0[k]-delta+nuP[i,m]
    Ut_enroll_grad_R[i,m]=lambdaPR_0[k]+nuR[i,m]+lambdaG0+selectivity_R_sim[i,m]
    Ut_enroll_grad_P[i,m]=lambdaPR_0[k]-delta+nuP[i,m]+lambdaG0+selectivity_P_sim[i,m]
...
## print dataset
Simulation_Output=Array{Union{Missing, Float64},3}(undef,size_N,size_shocks,84)
for m=1:1:size_shocks 
   for i=1:1:size_N
   # identifiers
    ...
    Simulation_Output[i,m,18]=Pr_persist_sim[i,m]               # X TRUE prob of persisting 
    ...
    Simulation_Output[i,m,81]=Ut_enroll_dropout_R[i,m]
    Simulation_Output[i,m,82]=Ut_enroll_dropout_P[i,m]
    Simulation_Output[i,m,83]=Ut_enroll_grad_R[i,m]
    Simulation_Output[i,m,84]=Ut_enroll_grad_P[i,m]
...
# Save baseline effort for other counterfactuals
df_eff = DataFrame(Eff_sim, :auto)
CSV.write(joinpath(path_processed, "Eff_sim_baseline.csv"), df_eff; writeheader=false)
