...
* DO-FILE DESCRIPTION:
* This do file estimates from the data the parameters of the auxiliary models of the 
* Generalised Indirect Inference method.
...
saveold "$dataClean/empirical_coefficients_rescale.dta", replace
...
* Match coeff on interaction treatment X perceived distance cutoff (1)
	*reg sim_hours_study  Txperceived_dist treatment perceived_dist_cutoff  c.perceived_dist_cutoff##c.($c_initial_cond) c.perceived_dist_cutoff##i.($i_initial_cond)    if hours_study!=. & perceived_dist_cutoff!=. ,  cluster(rbd_basefinal) // NB condition on nonmissing hours study and perceived distance in data. Perceived distance from data as regressor; constructed in lines 121-123
...
use "$dataClean/empirical_coefficients_rescale.dta", clear
