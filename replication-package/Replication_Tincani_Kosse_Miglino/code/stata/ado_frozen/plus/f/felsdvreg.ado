* 1.8.4 TC 12 may 2018 (bug fix for table with "number of movers per firm" which previously showed number of mover-years per firm)
...
timer clear
timer on 20
tempvar itemp jtemp jtemp2 miss sample n firmnum feffgbar f p pf gmax jmin moverp moverpf firm mnumcat cltemp
...
qui gen `moverpf' = `mover' if `pf' ==1
sort `jtemp2'
qui by `jtemp2': egen `mnum'=sum(`moverpf')
qui drop `moverpf'
qui by `jtemp2': gen `f'=1 if _n==1
if "`noisily'"=="noisily" {
